% PROCESSEMBRYO   Computes stripe boundary locations, intensities and 
% number of nuclei for each PCE and saves to individual .measure files.
% Also finds stripe peak locations and intensitites and saves to .peak
% files.
%
% Generally, called by a summary script.
%
% PROCESSEMBRYO(PCE_DIR, 'CHANNEL', NUMSTRIPES, INIT, [APmin APmax]) 
% reads in all pointclouds from PCE_DIR and computes the locations of 
% NUMSTRIPES stripes in 'CHANNEL' between APmin and APmax (normalized to 
% a/p length 1). 
% Note [APmin APmax] is an optional input, default is [0.2 .99].
% 
% Saved variables in .measure file:
%  pcname
%  cohort
...
%
% Saved variables in .peak file:
%  pcname
%  cohort
%  pos -> (16xNUMSTRIPES matrix) [stripe1_peak stripe2_peak... ]
%  vals -> (16xNUMSTRIPES matrix) [stripe1_peak stripe2_peak... ]
%
% Tara Martin, Jan 2013
% Adapted from PROCESSSTRIPES which is adapted from DEMO_EVESTRIPES 
%  (Cris Luengo, Jan-Feb 2009)

function ProcessEmbryo(dirname, channel, Nstripes, peak_guess, APthresh)
...
for i=1:length(files)
    ...
    thresholdedPC = segmentgap(pattern,Nstripes, APthresh);
    ...
    pos = locateapboundaries(onedim_gauss,init,polarities);
...
%     thresholdedPC
    ...
    switch lower(storeEmbryo)
        ...
        otherwise
            % Process peaks as well
            init = peak_guess(:,:,cohort);
            [pos2 vals] = locateappeaks(onedim_gauss, init);
            
            pos2 = [pos2(9:end,:);pos2(1:8,:)];    %re-order position matrix
            vals = [vals(9:end,:);vals(1:8,:)];    %also re-order values matrix
            
            % then plot
            figure(fig1);
            subplot(2,1,2)
            plot(pos2,repmat(phi,1,Nstripes),'rx-','linewidth',1);
            xlim([0 1]);
            
            % Ask user to approve embryo and store results
            storeEmbryo = input('Peaks look okay? (Y/N) (default=Y)', 's');
