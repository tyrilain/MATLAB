...
axh = obj(1);
while ~strcmp(get(axh,'type'),'axes')
   axh = get(axh,'parent');
end
...
sza = get(axh,'pos'); sza = sza(3:4);
xlim = get(axh,'xlim');
ylim = get(axh,'ylim');
islx = strcmp(get(axh,'xscale'),'log');
isly = strcmp(get(axh,'yscale'),'log');
...
ax=axis(axh);
lh = line('xdata',xi,'ydata',yi,'parent',axh);
set(lh,'linewidth',width);
set(lh,'color',color)
set(lh,'linestyle',style)
set(axh,'units',oldu)   % Set axes units back
axis(axh,ax);
